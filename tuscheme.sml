  ...
  let
    ...
    val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
      ( (* primitive functions for \tuscheme\ [[::]] 381d *)
        ...
        :: ("*", arithOp op*, arithtype) :: ("/", arithOp op div, arithtype)
        ...
        , unaryOp
            (fn (PAIR (car, _)) => car
              | v =>
               raise RuntimeError ("car applied to non-list " ^ valueString v))
        ...
        , unaryOp
            (fn (PAIR (_, cdr)) => cdr
              | v =>
               raise RuntimeError ("cdr applied to non-list " ^ valueString v))
        , FORALL (["'a"], FUNTY ([listtype tvA], listtype tvA))
        )
        :: (* primitive functions for \tuscheme\ [[::]] S407b *)
        ("<", intcompare op<, comptype) :: (">", intcompare op>, comptype)
        ::
        ( "="
